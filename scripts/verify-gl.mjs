...
void main() { fragColor = vec4(v_col, 1.0); }`;
    ...
    for (const [name, seed, zoom] of [["fit", 7, 0.6], ["mid", 7, 3.0], ["zoom", 21, 9.0]]) {
      ...
      wasm.system_set_night_lod(sys, 0);   // the GPU renders F_ALL — see `gl_feat`
      const acc = { total: 0, diff: 0, big: 0, sum: 0, worst: 0 };
      let bodies = 0;
...
function report(title, out, nameOf) {
  ...
  // Under the rate limit but not zero: a handful of pixels can differ by a lot
  // without moving the rate, so say so rather than letting the summary below
  // claim every difference was a threshold flip.
  if (worstBig > 0) {
    console.warn(`  note: ${(100 * worstBig).toFixed(4)}% of pixels differ by more than one level (max ${worstMax}/255) — under the ${(100 * LIMIT).toFixed(1)}% limit, but not a quantizer flip.`);
  }
  return { ok: true, clean: worstBig === 0 };
}
...
let ok = true, clean = true;
const note = r => { ok = !!(r && r.ok) && ok; clean = !!(r && r.clean) && clean; };
if (DEMO === "planet" || DEMO === "all") {
  ...
  note(report("planet", out, r => PLANET_NAMES[+r.label] ?? `type ${r.label}`));
}
if (DEMO === "solar" || DEMO === "all") {
  const out = await checkSolar(page, wasmOf("solar"), DEMO === "all" ? 220 : SIZE);
  if (!out.fatal) console.log(`renderer: ${out.renderer}`);
  note(report("solar", out, r => r.label));
}
await browser.close();
...
console.log(clean
  ...
  : "\nOK — within the limit, but see the notes above: some pixels differ by more than one level.");
