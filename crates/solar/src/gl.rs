...
//! order. Every number in it comes from the same expressions `draw_bodies`
...
pub fn gl_bodies(
    ...
    // Same draw list `draw_bodies` builds: the sun at depth 0, planets
    ...
    for &(_, which) in order.iter() {
        ...
        let size = if which < 0 {
            ...
            let (x0, y0) = dest_origin(cx, cy, size, scale);
            head[1..4].copy_from_slice(&[x0 as f32, y0 as f32, (size as f32 * scale).round().max(1.0)]);
            // `F_ALL`, not the scene's mask: `F_NIGHT_LOD` buys a CPU octaves it
            // cannot afford and moves pixels doing it, and a rasterizer does not
            // need the trade. See `gl_feat`.
