...
//
// The GL path runs the LIVE shader: no `F_BAKED_*`. Those exist to make the CPU
// afford the weather, and a GPU would rather evaluate the fBm than carry ~7.5 MB
...
mod tests {
    ...
    fn glsl_slot_indices_match_the_rust() {
        ...
        // Rust constants; only these five reach the GPU (the `F_BAKED_*` ones
        // have no GL path — the fragment shader evaluates the fBm live).
