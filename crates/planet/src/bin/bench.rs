...
use planet::{
    param, render_rgba_features, render_rgba_styled, type_count, type_name, F_ALL,
    F_BAKED_CLOUDS, NUM_PARAMS,
};
...
fn main() {
    ...
    // Baked (frozen) cloud deck. Interleaved against the live path rather than
    // run in two blocks: this machine's timings drift within a run, and the
    // whole point of the number is the ratio between the two.
    println!("\n=== baked clouds (F_BAKED_CLOUDS) ===");
    println!("{:<12} {:>5}  {:>9}  {:>9}  {:>7}", "type", "size", "live", "baked", "speedup");
    for &t in &["terran", "ocean", "gaia", "swamp", "toxic", "storm_shroud"] {
        for &size in &[64u32, 256] {
            let n = if size <= 64 { 200 } else { 30 };
            let (mut live, mut bake) = (0.0, 0.0);
            for _ in 0..3 {
                live += feat_bench(size, idx(t), F_ALL, n);
                bake += feat_bench(size, idx(t), F_ALL | F_BAKED_CLOUDS, n);
            }
            println!(
                "{:<12} {:>5}  {:>6.3} ms  {:>6.3} ms  {:>6.2}x",
                t, size, live / 3e6, bake / 3e6, live / bake
            );
        }
    }
}
