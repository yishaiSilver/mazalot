...
//! Dependency-free: no `image`, no `rand`. Produces raw RGBA via `render_rgba`;
//! the native binary wraps that into GIF/PNG, the web crate blits it to a canvas.
...
impl Rng {
    ...
    pub fn f32(&mut self) -> f32 {
        (self.next() >> 40) as f32 / (1u64 << 24) as f32 // [0,1)
    }
    pub fn chance(&mut self, p: f32) -> bool {
        self.f32() < p
    }
    pub fn range<R: RngRange>(&mut self, r: R) -> R::Out {
        r.sample(self)
    }
}
pub trait RngRange {
...
impl RngRange for Range<u32> {
    type Out = u32;
    fn sample(self, rng: &mut Rng) -> u32 {
        self.start + (rng.next() as u32) % (self.end - self.start)
    }
}
impl RngRange for RangeInclusive<u32> {
    type Out = u32;
    fn sample(self, rng: &mut Rng) -> u32 {
        *self.start() + (rng.next() as u32) % (*self.end() - *self.start() + 1)
    }
}

// ---------------- tiny RGBA image buffer (replaces image::RgbaImage) ----------------
struct Img {
    w: u32,
    h: u32,
    px: Vec<u8>,
...
impl Img {
    fn new(w: u32, h: u32) -> Self {
        Img { w, h, px: vec![0u8; (w * h * 4) as usize] }
    }
    fn put_pixel(&mut self, x: u32, y: u32, c: [u8; 4]) {
...
fn to_rgba(c: Rgb) -> [u8; 4] {
    [(clamp01(c[0]) * 255.0) as u8, (clamp01(c[1]) * 255.0) as u8, (clamp01(c[2]) * 255.0) as u8, 255]
}
...
fn r(rng: &mut Rng, a: f32, b: f32) -> f32 {
    a + (b - a) * rng.f32()
}
...
fn sample_genus(rng: &mut Rng) -> Genus {
    let el = r(rng, 1.5, 3.0);
    // pick two DISTINCT plan endpoints to hybridize
    let ia = rng.range(0..PLANS.len() as u32) as usize;
    let ib = (ia + rng.range(1..PLANS.len() as u32) as usize) % PLANS.len();
    Genus {
        plan_a: PLANS[ia],
        plan_b: PLANS[ib],
        // bias toward the extremes: one archetype dominates, the other twists it
        blend: if rng.chance(0.5) { r(rng, 0.12, 0.35) } else { r(rng, 0.65, 0.88) },
        hue_c: rng.f32(),
        hue_spread: r(rng, 0.02, 0.10),
        sat: (r(rng, 0.5, 0.7), r(rng, 0.75, 0.98)),
        val: (r(rng, 0.45, 0.6), r(rng, 0.7, 0.85)),
        scheme: rng.range(0..3),
        gear: [Gear::None, Gear::Antennae, Gear::Horns, Gear::Crest, Gear::Frill][rng.range(0..5) as usize],
        pat: [Pattern::Solid, Pattern::Solid, Pattern::Spots, Pattern::Stripes, Pattern::Dorsal, Pattern::Irid][rng.range(0..6) as usize],
        eyes: [1u32, 2, 2, 3][rng.range(0..4) as usize],
        eyes_vertical_p: if rng.chance(0.35) { 0.8 } else { 0.1 },
        eye_r: (1.0 + el * 0.2, 1.2 + el * 0.5),
        tentacle_p: if rng.chance(0.3) { 0.85 } else { 0.1 },
        mutation: r(rng, 0.12, 0.28),
        anim_energy: r(rng, 0.5, 1.5), // languid <-> twitchy family
        anim_tempo: if rng.chance(0.3) { 2 } else { 1 }, // some families move at double time
    }
}
...
fn skeleton(pp: &PP, rng: &mut Rng) -> Skel {
    let cx = 13.5;
...
fn sample(g: &Genus, rng: &mut Rng) -> Alien {
    ...
    let biolum = lp(ga, gb) > 0.4 || rng.chance(0.12);
    let fin = lp(fa, fb) > 0.4;

    // bioluminescent spots scattered on the body (seeded positions)
    let biolum_n = if biolum { rng.range(3..=7) } else { 0 };
    ...
    let biolum_col = if rng.chance(0.5) { hsv((rng.f32()).fract(), 0.75, 1.0) } else { [0.5, 1.0, 0.9] };

    // --- features (plan biases + genus signature + mutation) ---
    let mouth = mutate!(pp.mouth_kind, [Mouth::Beak, Mouth::Hook, Mouth::Tube, Mouth::Mandible, Mouth::Maw][rng.range(0..5) as usize]);
    let eyes = mutate!(g.eyes, [1u32, 2, 3][rng.range(0..3) as usize]);
    let mut gear = mutate!(g.gear, [Gear::None, Gear::Antennae, Gear::Horns, Gear::Crest, Gear::Frill][rng.range(0..5) as usize]);
    if pp.crest > 0.6 && rng.chance(0.7) {
        gear = Gear::Crest; // strongly-crested hybrids (macaw/peacock lineage)
    }
    let tail = mutate!(sk.tail_def, [Tail::None, Tail::Tuft, Tail::Fork, Tail::Fan, Tail::Spike, Tail::Whip][rng.range(0..6) as usize]);
    let pat = mutate!(g.pat, [Pattern::Solid, Pattern::Spots, Pattern::Stripes, Pattern::Dorsal, Pattern::Irid][rng.range(0..5) as usize]);
    ...
    let belly = if rng.chance(0.5) { lighter(back, 0.4) } else { hsv(h2u, r(rng, 0.4, 0.8), r(rng, 0.7, 0.95)) };
    ...
    let iris = if rng.chance(0.5) { hsv((hue + 0.5).fract(), 0.9, 1.0) } else { [0.98, 0.85, 0.25] };
    let beak_col = *[hsv((hue + 0.5).fract(), 0.8, 0.9), [0.9, 0.85, 0.7], [0.2, 0.18, 0.22]].get(rng.range(0..3) as usize).unwrap();
    let leg_col = if rng.chance(0.5) { darker(back, 0.35) } else { hsv((hue + 0.5).fract(), 0.7, 0.7) };

    let wing_stub = mutate!(rng.f32() < clamp01(pp.wing), rng.chance(0.5));
    let dorsal = rng.f32() < clamp01(pp.dorsal + 0.12);
    ...
    Alien {
        ...
        eyes,
        eyes_vertical: rng.f32() < g.eyes_vertical_p,
        eye_r: r(rng, g.eye_r.0, g.eye_r.1),
        gear,
        gear_n: rng.range(1..=3),
        dorsal,
        tail,
        tail_len: sk.tail_len,
        tail_half: sk.tail_half,
        legs: sk.legs_n,
        tentacle: rng.f32() < g.tentacle_p,
        leg_len: sk.leg_len,
        ...
        blink_frame: if rng.chance(0.6) { rng.range(0..ANIM_FRAMES) as i32 } else { -1 },
...
fn creature(genus_seed: u64, indiv: u64) -> Alien {
    let mut grng = Rng::seed(genus_seed);
    let g = sample_genus(&mut grng);
    let mut rng = Rng::seed(genus_seed.wrapping_mul(1009) + indiv + 1);
    sample(&g, &mut rng)
}
...
pub fn render_rgba(size: u32, seed: u32, phase: f32, energy: f32, detail: f32, out: &mut [u8]) {
    ...
    for oy in 0..size {
        ...
        for ox in 0..size {
            let sx_f = ox as f32 * inv;
            let di = ((oy * size + ox) * 4) as usize;
            let mut col = bg;
